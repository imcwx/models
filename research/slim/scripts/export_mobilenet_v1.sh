...
echo "*******"
echo python ~/Documents/gits/models/research/slim/export_inference_graph.py \
  --model_name=${SLIM_NAME} --image_size=${IMAGE_SIZE} --logtostderr \
  --output_file=${MODEL_FOLDER}/unfrozen_graph.pb --dataset_dir=${CHECKPOINT}

python ~/Documents/gits/models/research/slim/export_inference_graph.py \
  --model_name=${SLIM_NAME} --image_size=${IMAGE_SIZE} --logtostderr \
  --output_file=${MODEL_FOLDER}/unfrozen_graph.pb --dataset_name=np_dataset \
  --dataset_dir=${CHECKPOINT}

echo "*******"
echo "Freezing graph to ${MODEL_FOLDER}/frozen_graph.pb"
echo "*******"
OLD_CHECKPOINT=${CHECKPOINT}
CHECKPOINT=${CHECKPOINT}/train_dir/mobilenetv1/model.ckpt-146000
...
#LD_LIBRARY_PATH=/home/local/cuda-8.0/lib64 transform_graph \
...
python ~/Documents/gits/tensorflow/tensorflow/examples/label_image/label_image.py \
  ...
  --graph=${MODEL_FOLDER}/frozen_graph.pb --input_mean=-127 --input_std=127 \
  --image=/home/keyong/Downloads/PMI_Ukraine/classification_sample/sample_84126.jpg \
  --input_width=${IMAGE_SIZE} --input_height=${IMAGE_SIZE} --labels=${OLD_CHECKPOINT}/labels.txt
